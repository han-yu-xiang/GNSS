"""Run one immutable, new-only v2 darkroom parameter generation request.

The v2 runner consumes only a frozen request and the four already-derived
model families.  It never reads raw IQ, tracking, SAGE or MATLAB artifacts.
Validation is the default; generation requires an explicit confirmation and
always writes into a request-specific v2 namespace.
"""
...
LOCK_NAME = ".darkroom_generator_v2.active.lock"
...
BLOCK_FIELDS: tuple[str, ...] = (
    ...
    "latent_delay_ns",
    "latent_doppler_hz",
    "latent_relative_amplitude",
    "output_relative_amplitude_base",
    "phase_initial_rad",
    "slot_status",
...
def _validate_namespace(project_root: Path, relative_namespace: str, request_id: str) -> Path:
    ...
    if not normalized.startswith(prefix):
        raise ValueError("output namespace is outside the v2 run root")
...
def _current_source_hashes() -> dict[str, str]:
    ...
    paths = {
        "scripts/analysis/channel_modeling/darkroom_generator_v2_core.py": current.with_name("darkroom_generator_v2_core.py"),
        "scripts/analysis/channel_modeling/prepare_darkroom_generator_v2_request.py": current.with_name("prepare_darkroom_generator_v2_request.py"),
        "scripts/analysis/channel_modeling/run_darkroom_generator_v2.py": current,
        "scripts/analysis/channel_modeling/audit_darkroom_generator_v2.py": current.with_name("audit_darkroom_generator_v2.py"),
    }
...
def _validate_request(project_root: Path, request_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if not _is_within(request_path, request_root) or request_path.name != "generation_request.json":
        raise ValueError("request must be generation_request.json under the v2 request root")
    ...
    if sha256_file(config_path).lower() != str(request.get("generator_config_sha256", "")).lower():
        raise ValueError("generator config hash mismatch")
    config = load_v2_config(config_path, project_root)
    request_obj = validate_v2_request(request, config)
    output_relative = str(request.get("output_namespace", ""))
    if output_relative != str(request.get("expected_output_namespace", output_relative)):
    ...
    if declared_sources != current_sources:
        raise ValueError("v2 source hash mismatch")
    ...
    if lock_path.exists():
        raise RuntimeError(f"active v2 generator lock exists: {lock_path}")
    return {
        "request": request,
...
def _validate_generation_confirmation(generate: bool, confirm: bool) -> None:
    if generate and not confirm:
        raise ValueError("--generate requires --confirm-darkroom-generation-v2")
    if confirm and not generate:
        raise ValueError("--confirm-darkroom-generation-v2 requires --generate")
...
def _release_lock(lock_path: Path, request_id: str) -> None:
    if not lock_path.exists():
        return
    released = lock_path.with_name(f".darkroom_generator_v2.released.{request_id}.{time.time_ns()}.lock")
...
def _write_failure_receipt(context: Mapping[str, Any], status: str, error: str, started_utc: str) -> None:
    ...
    _write_json(
        receipt_path,
        {
            "receipt_schema_version": "darkroom-generator-receipt-2",
            "status": status,
            "error": error,
...
def _write_generation_report(path: Path, request: Mapping[str, Any], result: Any, receipt: Mapping[str, Any]) -> None:
    lines = [
        "# Darkroom Multi-Elevation Fixed-Four-Slot Generator v2 Preview",
        "",
        "This is a Python-only deterministic parameter preview generated from frozen derived model artifacts.",
        ...
        "Inactive NLOS rows retain finite latent delay, Doppler and phase and use `RelativeAmplitude=0`; these latent values are not physical paths.",
        "",
        "## Scientific status",
        "",
...
def _generate(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        result = generate_v2_simulation(context["request_obj"], context["config"], context["models"])
        ...
        (output_dir / "darkroom_channel_parameters.csv").write_bytes(format_v2_final_rows(result.final_rows).encode("utf-8"))
        ...
        manifest = {
            "manifest_schema_version": "darkroom-generator-manifest-2",
            ...
            "parameter_provenance": {
                "generator_config_sha256": request["generator_config_sha256"],
                "parent_generator_config_sha256": request["parent_generator_config_sha256"],
        ...
        receipt_base["output_files"] = output_files + ["generation_receipt.json"]
        receipt_base["output_hashes_excluding_receipt"] = output_hashes
        _write_json(output_dir / "generation_receipt.json", receipt_base)
        return receipt_base
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--confirm-darkroom-generation-v2", action="store_true")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        _validate_generation_confirmation(args.generate, args.confirm_darkroom_generation_v2)
        ...
        print("V2_GENERATOR_INTERRUPTED=KeyboardInterrupt")
        return 130
    except Exception as exc:
        print(f"V2_GENERATOR_REJECTED={type(exc).__name__}: {exc}")
        return 2
