function g06_nav_sage_pipeline
%G06_NAV_SAGE_PIPELINE Navigation-symbol-aided G06 multipath estimation.
%
% Place this file in the project root, for example:
%   F:\F1023_V70_D0117_P2
%
% Required input folder under the project root:
%   GNSS-SDR_Res
%
% Run:
...
%
% Processing stages:
%   0. Build a catalog from continuous, complete G06 telemetry symbols.
...
[rootDir, rawFile, resultDir, outputDir] = resolveInputs();

fprintf("G06 navigation-aided SAGE pipeline V%d\n", ...
    cfg.pipelineVersion);
...
telemetryFile = fullfile(resultDir, "telemetry", ...
    "F1023_V70_D0117_P2_telemetry_ch_4.dat");
trackingFile = fullfile(resultDir, "tracking", ...
    "F1023_V70_D0117_P2_track_ch_4.mat");
assert(isfile(telemetryFile), ...
    "Missing G06 telemetry file: %s", telemetryFile);
assert(isfile(trackingFile), ...
    "Missing G06 tracking file: %s", trackingFile);

telemetry = readTelemetryDat(telemetryFile);
tracking = readTrackingMat(trackingFile);
[nmeaUtcSod, nmeaSpeedKmh] = readNmeaSpeed(resultDir);
...
stage2ModelsCsv = fullfile(outputDir, ...
    "stage2_model_orders.csv");
stage2SelectedCsv = fullfile(outputDir, ...
    "stage2_selected_windows.csv");
stage2PathsCsv = fullfile(outputDir, ...
...
stage3ReliableCsv = fullfile(outputDir, ...
    "stage3_reliable_centers.csv");
stage4Mat = fullfile(outputDir, "stage4_nav_joint_100ms.mat");
stage4SummaryCsv = fullfile(outputDir, ...
    "stage4_joint_summary.csv");
stage4PathsCsv = fullfile(outputDir, ...
    "stage4_joint_paths.csv");
overviewFile = fullfile(outputDir, ...
    "G06_nav_sage_overview.png");

%% Stage 0
if cfg.resumeExistingStages && isfile(stage0Mat)
    ...
    assert(~isempty(windowCatalog), ...
        "No complete G06 40 ms windows were found.");
...
%% Doppler sign
dopplerSignFile = fullfile(outputDir, ...
    "doppler_sign.mat");
if cfg.resumeExistingStages && isfile(dopplerSignFile)
    loaded = load(dopplerSignFile, "dopplerSignUsed");
    dopplerSignUsed = loaded.dopplerSignUsed;
else
    dopplerSignUsed = determineDopplerSign( ...
        windowCatalog, rawFile, cfg);
    save(dopplerSignFile, "dopplerSignUsed");
end
fprintf("GNSS-SDR Doppler sign used: %+d\n", dopplerSignUsed);

%% Stage 1
stage1CanResume = cfg.resumeExistingStages ...
    && checkpointVersionMatches(stage1Mat, cfg.pipelineVersion);
if stage1CanResume
    loaded = load(stage1Mat, "stage1Table");
    stage1Table = loaded.stage1Table;
    if ismember('scan_valid', stage1Table.Properties.VariableNames) ...
            && any(stage1Table.scan_valid == 1)
...
if ~stage1CanResume
    ...
    stage1Table = runFastScan(windowCatalog, ...
        rawFile, dopplerSignUsed, outputDir, cfg);
    writetable(stage1Table, stage1Csv);
    save(stage1Mat, "stage1Table", ...
        "dopplerSignUsed", "cfg");
end
...
elseif any(failedStage1)
    warning("Stage 1 failed for %d of %d windows.", ...
        nnz(failedStage1), height(stage1Table));
end

candidateIndices = chooseStage2Candidates( ...
    stage1Table, windowCatalog, cfg);
fprintf("Stage 1 selected %d windows including neighbors.\n", ...
    numel(candidateIndices));

%% Stage 2
if cfg.resumeExistingStages ...
        && checkpointVersionMatches(stage2Mat, cfg.pipelineVersion)
    loaded = load(stage2Mat, "stage2Fits", ...
        "modelTable", "selectedTable", "pathTable");
    ...
    stage2Fits = runStage2(candidateIndices, ...
        windowCatalog, stage1Table, rawFile, ...
        dopplerSignUsed, outputDir, cfg);
    [modelTable, selectedTable, pathTable] = ...
        flattenStage2(stage2Fits, cfg);
    writetable(modelTable, stage2ModelsCsv);
    writetable(selectedTable, stage2SelectedCsv);
    writetable(pathTable, stage2PathsCsv);
    save(stage2Mat, "stage2Fits", "modelTable", ...
        "selectedTable", "pathTable", "cfg");
end

%% Stage 3
if cfg.resumeExistingStages ...
        && checkpointVersionMatches(stage3Mat, cfg.pipelineVersion)
    loaded = load(stage3Mat, ...
        "persistenceTable", "reliableTable");
    ...
    [persistenceTable, reliableTable] = ...
        evaluatePersistence(stage2Fits, ...
        windowCatalog, cfg);
    writetable(persistenceTable, stage3Csv);
    writetable(reliableTable, stage3ReliableCsv);
    save(stage3Mat, "persistenceTable", ...
        "reliableTable", "cfg");
end

%% Stage 4
if cfg.resumeExistingStages ...
        && checkpointVersionMatches(stage4Mat, cfg.pipelineVersion)
    loaded = load(stage4Mat, ...
        "jointFits", "jointSummaryTable", ...
        "jointPathTable");
    jointFits = loaded.jointFits; %#ok<NASGU>
    ...
    [jointFits, jointSummaryTable, jointPathTable] = ...
        runJointStage(reliableTable, stage2Fits, ...
        symbolCatalog, windowCatalog, rawFile, ...
        dopplerSignUsed, cfg);
    writetable(jointSummaryTable, stage4SummaryCsv);
    writetable(jointPathTable, stage4PathsCsv);
    save(stage4Mat, "jointFits", ...
        "jointSummaryTable", "jointPathTable", "cfg");
end
...
cfg.pipelineVersion = 2;
cfg.targetPrn = 6;
cfg.resumeExistingStages = true;
...
function [rootDir, rawFile, resultDir, outputDir] = resolveInputs()
...
addressFile = "";
for index = 1:numel(addressCandidates)
    if isfile(addressCandidates{index})
        addressFile = string(addressCandidates{index});
        break;
    end
end
assert(strlength(addressFile) > 0, ...
    "Cannot find data_address.txt.");
rawFile = strtrim(fileread(addressFile));
rawFile = erase(rawFile, '"');
rawFile = erase(rawFile, "'");
if ~isfile(rawFile)
...
resultDir = fullfile(rootDir, "GNSS-SDR_Res");
assert(isfolder(resultDir), ...
    "GNSS-SDR_Res folder not found: %s", resultDir);

outputDir = fullfile(rootDir, "sage_results", ...
    "G06_nav_sage_v1");
...
function [utcSod, speedKmh] = readNmeaSpeed(resultDir)
utcSod = [];
speedKmh = [];
files = dir(fullfile(resultDir, "nmea", "*.nmea"));
if isempty(files)
    return;
end
...
if cfg.resumeExistingStages ...
        && checkpointVersionMatches(progressFile, cfg.pipelineVersion)
...
function matches = checkpointVersionMatches(filename, expectedVersion)
matches = false;
if ~isfile(filename)
    return;
end
