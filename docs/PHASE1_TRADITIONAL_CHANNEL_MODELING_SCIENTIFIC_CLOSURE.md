...
Interaction is assessed as a difference-in-differences: the environment-specific elevation-band contrast is compared with the corresponding contrast in other environments, with scene-block bootstrap and leave-one-scene-out stability. Visual cell differences alone are not treated as interactions.
...
| Special Reflective | excess_delay_samples | LOW→HIGH | PARTIAL | SPARSE_PARTIAL_POOLING |
| Mountain/Valley | excess_delay_samples | LOW→HIGH | INCONCLUSIVE | SPARSE_PARTIAL_POOLING |
...
| Special Reflective | relative_power_db | LOW→HIGH | INCONCLUSIVE | SPARSE_PARTIAL_POOLING |
| Mountain/Valley | relative_power_db | LOW→HIGH | PARTIAL | SPARSE_PARTIAL_POOLING |
| Highway/Open | relative_power_db | MID→HIGH | PARTIAL | PRIOR_DOMINANT |
...
All closure outputs are in the new-only namespace `E:\GNSS_Multipath_Project\dataset_generation_logs\channel_modeling\phase1_scientific_closure_20260830_r2`. No MATLAB/SAGE/batch process, raw IQ read, 20.46 MHz processing, AI training, production request, or modification of r3/r1/r2/Stage4 was performed.
